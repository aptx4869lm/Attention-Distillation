...
\end{itemize}
%\textbullet\ \textbf{Soft-Atten} combines soft attention and attention pooling for recognition. This is used in~\cite{liu2018end}.\\
%\textbullet\ \textbf{Soft-Res} is the residual attention in~\cite{wang2017residual} that further adds residual connection to Soft-Atten. \\
%\textbullet\ \textbf{Prob-Atten} is the probabilistic modeling in~\cite{Li_2018_ECCV} that combines probabilistic attention with attention pooling.
...
\multirow{14}{*}{\begin{tabular}[c]{@{}c@{}}RGB Only\end{tabular}}      
& \makecell{Two Stream I3D\text{*}~\cite{carreira2017quo} }             & 96.8      & 76.1            \\ 
\hline
...
& \makecell{I3D RGB\text{*} (backbone)~\cite{carreira2017quo} }         & 94.8              & 70.9            \\
& \makecell{Ours (RGB)}                                                 & \textbf{95.7}     & \textbf{72.0}            \\
\hdashline
\multirow{3}{*}{\begin{tabular}[c]{@{}c@{}}More Frames\end{tabular}}  
& \makecell{I3D RGB (64) ~\cite{carreira2017quo} }                           & 95.6              & 74.8          \\
& \makecell{R(2+1)D RGB (32)~\cite{Tran_2018_CVPR}}                          & 96.8              & 74.5            \\
& \makecell{S3D (64)~\cite{Xie_2018_ECCV}}                                   & 96.8              & 75.9            \\
\end{tabular}
\vspace{0.1em}
\caption{Results of action recognition on UCF101 and HMDB51. We compare the results of our model with several previous methods. Our model outperforms state-of-the-art results that use single RGB stream and the same input sequence length by $\app 1\%$. \text{*}For fair comparison, we report results of I3D models that use 24 frames as inputs--the same as our model.\vspace{-0.5em}}
\label{table:action}
\end{table}
